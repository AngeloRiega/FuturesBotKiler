...
namespace FuturesBotKiler.Controllers
{
    ...
    public class BinanceController : ControllerBase
    {
        ...
        {
            if (await Program.ClosePosition(symbol))
            {
                return Ok(true);
            }
            else
        ...
        public async Task<ActionResult<string>> AlertaTradingView()
        {
            ...
            if (tradingViewAlert is not null)
            {
                    ...
                    tradingViewAlert.PorcentajeTakeProfit != 0)
                {
                    ...
                    TelegramMessage.Message($"ALERTA TRADINGVIEW {tradingViewAlert.Symbol} - {orderSide}");
                ...
                TelegramMessage.Message($"ERROR PARAMETROS TRADINGVIEW ALERTA");
                return BadRequest("ERROR PARAMETROS TRADINGVIEW ALERTA");
            }
            else
            {
                TelegramMessage.Message($"ERROR PARAMETROS TRADINGVIEW ALERTA");
